...
public class PachetTuristic implements IPachetTuristic {
    ...
    public void descrierePachet(Optionale optionale) {
        System.out.println("Pachetul Turistic ce are codul " + codPachet
                + ", are cazarea la hotelul " + Hotel
                + ", din orasul " + destinatie
                + (micDejun ? " are" : "nu are")
                + "mic dejun, " + (optionale.isCina() ? " are" : " nu are")
                + "inclusa cina si are un numar de " + optionale.getNumarExcursii() + " excursii optionale");
    }
}
